...
def list_samples(root: str) -> List[str]:
    ...
    for d in sorted(glob.glob(os.path.join(root, "*"))):
        ...
        if sid == "cohort_summary":  # Skip summary directory
            continue
        if os.path.exists(os.path.join(d, "ivar_only_final.tsv")) or os.path.exists(
            os.path.join(d, f"{sid}.ivar_mid.tsv")
        ):
            sids.append(sid)
    return sids
...
def parse_clair3_vcf(vcf_path: str) -> pd.DataFrame:
    ...
    if _HAS_PYSAM:
        ...
        for rec in vf:
            filt = (
                "PASS"
                if (len(rec.filter.keys()) == 0 or "PASS" in rec.filter.keys())
                else ",".join(list(rec.filter.keys()))
            )
            if filt != "PASS":
                continue
                ...
                if sm is not None and sm in rec.samples:
                    ...
                    if "AD" in s and s["AD"] is not None:
                        try:
                            ...
                            if len(ad) >= 2:
                                AD = (
                                    int(ad[alts.index(alt) + 1])
                                    if len(ad) > alts.index(alt) + 1
                                    else None
                                )
    ...
    with opener(vcf_path, "rt") as fh:
        for line in fh:
            ...
            fmt_map = (
                {
                    k: (sample_fields[i] if i < len(sample_fields) else None)
                ...
                rows.append(
                    {
                        ...
                        "alt": a,
                        "filter": "PASS",
                        "qual": float(qual) if qual not in (None, ".") else None,
                        "AF_clair3": AF,
                        "DP_clair3": DP,
                        "AD_clair3": AD,
                        "var_type": vt,
...
def main():
    ...
    for sid in sids:
        ...
        iv_cand = (
            df_iv[df_iv["decision"] == "CANDIDATE"].copy() if not df_iv.empty else pd.DataFrame()
        )

        # Clair3 PASS events
        c3 = df_c3.copy()
        if not c3.empty:
            c3["event_key"] = c3.apply(
                lambda r: make_event_key(r["chrom"], r["pos"], r["ref"], r["alt"]), axis=1
            )
        ...
        if not c3.empty:
            c3["sample_id"] = sid
            c3["source_label"] = c3["event_key"].apply(
                lambda k: "DUAL" if k in iv_all_keys else "CLAIR3_ONLY"
            )
            c3["tier"] = "CLAIR3-PASS"
            c3["decision"] = "KEEP"
            ...
            c3["AF_ivar"] = c3["event_key"].apply(
                lambda k: iv_all_idx.get(k, (None, None, None))[0]
            )
            c3["DP_ivar"] = c3["event_key"].apply(
                lambda k: iv_all_idx.get(k, (None, None, None))[1]
            )
            c3["AD_ivar"] = c3["event_key"].apply(
                lambda k: iv_all_idx.get(k, (None, None, None))[2]
            )
    ...
    if not cohort_acc.empty:
        ...
        agg = cohort_acc.groupby(grp_cols, as_index=False).agg(
    ...
    def summarize_sites(df: pd.DataFrame, label: str) -> pd.DataFrame:
        ...
        for pos_ref, meta in TARGET_SITES.items():
            ...
            if "AF_primary" in sub.columns:
                ...
                af_ivar_col = (
                    sub["AF_ivar"] if "AF_ivar" in sub.columns else pd.Series([None] * len(sub))
                )
    ...
    if not cohort_acc.empty:
        ...
        bcp_prec_acc.to_csv(
            os.path.join(OUT_DIR, "bcp_prec_summary_tier1.tsv"), sep="\t", index=False
        )

    if not cohort_cand.empty:
        bcp_prec_cand = summarize_sites(cohort_cand, "Tier2")
        bcp_prec_cand.to_csv(
            os.path.join(OUT_DIR, "bcp_prec_summary_tier2.tsv"), sep="\t", index=False
        )
