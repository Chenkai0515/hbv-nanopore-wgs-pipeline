...
def filter_bam_by_mismatch(
    input_bam, output_bam, mismatch_thresh=50, filter_end=False, min_length=500
):
    """
    ...
    for read in infile:
        ...
        if filter_end:
            ...
            if md_tag and read_seq:
                tail_mismatches = analyze_md_tag(
                    md_tag, read_seq, mismatch_thresh, mismatch_thresh, True
                )
...
def process_single_bam(input_bam, mismatch_thresh=50, filter_end=False, min_length=500):
    ...
    try:
        ...
        print(
            f"Filter parameters: mismatch_thresh={mismatch_thresh}, filter_end={filter_end}, min_length={min_length}bp"
        )
        print(f"{'='*60}")
        ...
        print(
            f"Filtering {standard_filtered_bam}, removing reads with long mismatches at {filter_description} (threshold: {mismatch_thresh} bp)..."
        )
        print(f"Also filtering out reads shorter than {min_length}bp...")

        stats = filter_bam_by_mismatch(
            standard_filtered_bam, final_bam, mismatch_thresh, filter_end, min_length
        )
        ...
        final_stats = os.path.join(
            sample_output_dir, f"{sample_id}.final.filtered.sorted.bam.flagstat.txt"
        )
...
def main():
    ...
    parser.add_argument("--filter_end", action="store_true", help="Also filter the end regions")
    parser.add_argument(
        "--min_length", type=int, default=500, help="Minimum read length (default: 500)"
    )
    parser.add_argument(
        "--jobs",
        "-j",
    ...
    print(
        f"Filter parameters: mismatch_thresh={args.mismatch_thresh}, filter_end={args.filter_end}, min_length={args.min_length}bp"
    )
    print("=" * 60)
    ...
    if args.sample:
        ...
        success = process_single_bam(
            input_bam, args.mismatch_thresh, args.filter_end, args.min_length
        )
        ...
        if args.jobs > 1:
            ...
            for bam_file in bam_files:
                if process_single_bam(
                    bam_file, args.mismatch_thresh, args.filter_end, args.min_length
                ):
                    success_count += 1
                else:
                    failed_count += 1
