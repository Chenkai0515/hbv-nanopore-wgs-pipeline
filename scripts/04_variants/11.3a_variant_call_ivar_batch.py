...
CONSENSUS_DIR = os.environ.get(
    "CONSENSUS_DIR", os.path.join(PROJECT_DIR, "Medaka_consensus_8", "r2")
)
...
def run_variant_call(
    ...
    if not force and os.path.exists(variants_tsv) and os.path.getsize(variants_tsv) > 0:
        print(
            f"[{sample_id}] Detected existing output. Use --force to rerun or delete existing results"
        )
    ...
    if not os.path.exists(consensus_fasta):
        print(
            f"[Warning] Consensus sequence file does not exist: {consensus_fasta}, skipping sample {sample_id}"
        )
        return False
    ...
    cmd_mpileup = [
        ...
        "-A",
        "-d",
        str(int(mp_max_depth)) if int(mp_max_depth) > 0 else "0",
        "-Q",
        str(int(mp_base_qual)),  # Changed to -Q 15 (default), filters low base quality to speed up
        "-B",
        "-f",
        consensus_fasta,
    ]
    ...
    cmd_ivar = [
        ...
        str(MIN_FREQ),
        "-p",
        variants_tsv.replace(".tsv", ""),  # ivar will automatically add .tsv / .tsv.indel
    ]
    ...
    try:
        ...
        p2 = subprocess.Popen(
            cmd_ivar, stdin=p1.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        p1.stdout.close()
        ...
        if p2.returncode != 0:
            print(
                f"[Error] ivar variants failed for sample {sample_id}. Error message: {stderr.decode('utf-8', errors='ignore')}"
            )
            return False
...
def test_single_sample(sample_id, args):
    ...
    if sample_id not in valid_samples:
        ...
        print(
            f"Available samples: {', '.join(valid_samples[:10])}{'...' if len(valid_samples) > 10 else ''}"
        )
        return False
...
def main():
    ...
    parser.add_argument(
        "--threads",
        type=int,
        default=MP_THREADS_DEFAULT,
        help="Reserved thread parameter (samtools mpileup doesn't support multithreading)",
    )
    parser.add_argument(
        "--jobs",
        "-j",
    ...
    parser.add_argument(
        ...
        "-D",
        type=int,
        default=MP_MAX_DEPTH_DEFAULT,
        help="mpileup maximum depth limit (-d, default 0 = unlimited)",
    )
    parser.add_argument(
        "--baseq",
        "-Q",
        type=int,
        default=MP_BASE_QUAL_DEFAULT,
        help="mpileup minimum base quality (-Q, default 15)",
    )
    parser.add_argument(
        "--mapq",
        "-q",
        type=int,
        default=MP_MAP_QUAL_DEFAULT,
        help="mpileup minimum mapping quality (-q, default 10)",
    )
    ...
    parser.add_argument(
        "--skip-existing", action="store_true", help="Skip sample if output already exists"
    )
    ...
    print(
        f"mpileup parameters: -Q {args.baseq}, -q {args.mapq}, -d {args.max_depth if args.max_depth>0 else 'unlimited'}"
    )
    ...
    if args.all:
        ...
        if args.jobs <= 1:
            ...
            for sample_id in sample_ids:
                ...
                finished += 1
                print(
                    f"Progress: {finished}/{total} (success: {success_count}, failed: {fail_count})"
                )
        else:
            # Parallel
            print(
            ...
            print(
                f"[Info] Will process {args.jobs} samples simultaneously, each sample's mpileup is single-threaded."
            )
            ...
            with ThreadPoolExecutor(max_workers=args.jobs) as ex:
                ...
                for fut in as_completed(future_to_sample):
                    ...
                    try:
                        ...
                        print(
                            f"[Error] Sample {sid} raised exception during parallel execution: {e}"
                        )
                        ok = False
