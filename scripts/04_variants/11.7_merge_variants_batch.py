...
def get_sample_directories():
    ...
    for sample_dir in input_path.iterdir():
        ...
        if files_found > 0:
            sample_dirs[sample_dir.name] = {"input_dir": sample_dir, "files": sample_files}
...
def merge_sample_files(sample_id, sample_info):
    ...
    try:
        ...
        print(
            f"[OK] {sample_id}: {merge_info['total_variants']} variants → {merge_info['merged_variants']} rows"
        )
        return merge_info
...
def generate_merge_summary(results):
    ...
    try:
        with open(summary_file, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([])
            writer.writerow(
                ["Sample ID", "accepted variants", "candidates variants", "Total", "Output file"]
            )

            for result in results:
                if result:
                    accepted_count = (
                        result["files_processed"].get("accepted", {}).get("variants_count", 0)
                    )
                    candidates_count = (
                        result["files_processed"].get("candidates", {}).get("variants_count", 0)
                    )
                    writer.writerow(
                        [
                            result["sample_id"],
        ...
        total_accepted = sum(
            r["files_processed"].get("accepted", {}).get("variants_count", 0) for r in successful
        )
        total_candidates = sum(
            r["files_processed"].get("candidates", {}).get("variants_count", 0) for r in successful
        )
        ...
        with open(stats_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"Average per sample: {total_merged / len(successful):.1f}\n"
                if successful
                else "Average: 0\n"
            )
