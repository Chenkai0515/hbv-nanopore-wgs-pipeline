        ...
        with open(input_file, "r", encoding="utf-8") as infile, open(
            ...
            for row in reader:
                try:
                    ...
                    print(
                        f"[WARN] {sample_id} {file_type}: Cannot process pos '{row.get('pos', 'N/A')}': {e}"
                    )
                    continue
    ...
    try:
        with open(summary_file, "w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                [
                    "Formula",
                    f"original_pos = ((current_pos - 1) + {DEFAULT_OFFSET}) % {GENOME_LENGTH} + 1",
                ]
            )
            writer.writerow([])
            writer.writerow(
                ["Sample ID", "accepted.tsv variants", "candidates.tsv variants", "Total variants"]
            )

            for result in results:
                if result:
                    accepted_count = (
                        result["files_processed"].get("accepted.tsv", {}).get("variants_count", 0)
                    )
                    candidates_count = (
                        result["files_processed"].get("candidates.tsv", {}).get("variants_count", 0)
                    )
                    writer.writerow(
                        [
                            result["sample_id"],
