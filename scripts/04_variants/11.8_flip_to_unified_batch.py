...
def main():
    ...
    for bp in b_files:
        ...
        sample_id = (
            str(B["sample_id"].iloc[0])
            if not B.empty
            else (
                re.findall(r"\d+", os.path.basename(bp))[0]
                if re.findall(r"\d+", os.path.basename(bp))
                else ""
            )
        )
        ...
        if len(cand) > 1:
            print(
                f"[WARN] Multiple View A CSVs for sample {sample_id}, using first: {os.path.basename(cand[0])}"
            )
    ...
    if summary:
        S = pd.DataFrame(
            summary, columns=["sample_id", "input_b", "output_unified", "n_in", "n_out"]
        )
        print("\n" + "=" * 60)
