...
WORK_DIR = os.environ.get("HBV_WORK_DIR", os.path.join(_PROJECT_DIR, "variants_call_10"))
...
PROFILES = {
    "strict": {
        ...
        "SNP_LF_AD_MIN": 30,
        # INDEL
        "INDEL_HC_AF_MIN": 0.10,
        "INDEL_HC_DP_MIN": 500,
        "INDEL_HC_AD_MIN": 50,
        "INDEL_MC_DP_MIN": 1000,
        "INDEL_MC_AD_MIN": 25,
        "INDEL_LF_DP_MIN": 3000,
        "INDEL_LF_AD_MIN": 40,
        # Quality/strand/homopolymer
        "ALT_QUAL_MIN": 25,
        "STRAND_MIN_READS": 5,
        "STRAND_MIN_FRAC": 0.20,
        "HMER_FAIL_LEN_SNP": 6,
        "HMER_FAIL_LEN_INDEL": 6,
        # Structure length
        "INS_MAX_HC": 10,
        "DEL_MAX_HC": 33,
        "INS_MAX_LF": 5,
    ...
    "balanced": {
        ...
        "INDEL_HC_AF_MIN": 0.12,
        "INDEL_HC_DP_MIN": 500,
        "INDEL_HC_AD_MIN": 40,
        "INDEL_MC_DP_MIN": 800,
        "INDEL_MC_AD_MIN": 25,
        "INDEL_LF_DP_MIN": 2500,
        "INDEL_LF_AD_MIN": 35,
        "ALT_QUAL_MIN": 20,
    ...
    "lenient": {
        ...
        "INDEL_MC_DP_MIN": 600,
        "INDEL_MC_AD_MIN": 20,
        "INDEL_LF_DP_MIN": 2000,
        "INDEL_LF_AD_MIN": 30,
        "ALT_QUAL_MIN": 20,
        "STRAND_MIN_READS": 4,
        "STRAND_MIN_FRAC": 0.15,
        "HMER_FAIL_LEN_SNP": 6,
        "HMER_FAIL_LEN_INDEL": 6,
        "INS_MAX_HC": 12,
        "DEL_MAX_HC": 40,
...
def strand_eval_status(alt_fwd: float, alt_rev: float) -> str:
    ...
    if total < 2 * P["STRAND_MIN_READS"]:
        return "unevaluable"
    if (
        alt_fwd >= P["STRAND_MIN_READS"]
        and alt_rev >= P["STRAND_MIN_READS"]
        and min(alt_fwd, alt_rev) / max(1, (alt_fwd + alt_rev)) >= P["STRAND_MIN_FRAC"]
    ):
        return "ok"
    return "fail"
...
def classify_row(r: pd.Series) -> TierResult:
    ...
    if vt == "SNP":
        ...
        if (
            AF >= P["AF_HIGH"]
            and DP >= P["SNP_HC_DP_MIN"]
            and AD >= max(P["SNP_HC_AD_MIN"], int(0.15 * DP))
            and HMER < P["HMER_FAIL_LEN_SNP"]
            and (sg in (None, "strand_unevaluable", "strand_fail_but_covered"))
        ):
        ...
        if (
            P["AF_MID_LOW"] <= AF < P["AF_HIGH"]
            and DP >= P["SNP_MC_DP_MIN"]
            and AD >= max(P["SNP_MC_AD_MIN"], int(0.03 * DP))
            and HMER < P["HMER_FAIL_LEN_SNP"]
            and (sg in (None, "strand_unevaluable", "strand_fail_but_covered"))
        ):
        ...
        if (
            P["AF_LOW"] <= AF < P["AF_MID_LOW"]
            and DP >= P["SNP_LF_DP_MIN"]
            and AD >= P["SNP_LF_AD_MIN"]
            and HMER <= 4
            and (sg in (None, "strand_unevaluable", "strand_fail_but_covered"))
        ):
            return TierResult(
                "SNP-LF", "CANDIDATE", [] if sg in (None, "strand_unevaluable") else [sg]
            )
        ...
        if AF < P["AF_LOW"]:
            why.append("af_low")
        if DP < min(P["SNP_LF_DP_MIN"], P["SNP_MC_DP_MIN"]):
            why.append("dp_low")
        if AD < min(P["SNP_LF_AD_MIN"], P["SNP_MC_AD_MIN"]):
            why.append("ad_low")
        if HMER >= P["HMER_FAIL_LEN_SNP"]:
            why.append("homopolymer")
        if strand_status == "fail":
            why.append("strand_fail")
    ...
    if vt == "INS":
        ...
        # High confidence
        if (
            AF >= P["INDEL_HC_AF_MIN"]
            and DP >= P["INDEL_HC_DP_MIN"]
            and AD >= max(P["INDEL_HC_AD_MIN"], int(0.08 * DP))
            and (HMER < P["HMER_FAIL_LEN_INDEL"] or (AF >= 0.30 and DP >= 1000))
            and LEN <= P["INS_MAX_HC"]
            and (sg in (None, "strand_unevaluable", "strand_fail_but_covered"))
        ):
            return TierResult(
                "INDEL-HC", "KEEP", [] if sg in (None, "strand_unevaluable") else [sg]
            )

        # Medium confidence
        sg = strand_guard(ad_needed=60, af_needed=0.08)
        if (
            P["AF_MID_LOW"] <= AF < P["INDEL_HC_AF_MIN"]
            and DP >= P["INDEL_MC_DP_MIN"]
            and AD >= P["INDEL_MC_AD_MIN"]
            and HMER < P["HMER_FAIL_LEN_INDEL"]
            and LEN <= P["INS_MAX_HC"]
            and (sg in (None, "strand_unevaluable", "strand_fail_but_covered"))
        ):
            return TierResult(
                "INDEL-MC", "KEEP", [] if sg in (None, "strand_unevaluable") else [sg]
            )
        ...
        if (
            P["AF_LOW"] <= AF < P["AF_MID_LOW"]
            and DP >= P["INDEL_LF_DP_MIN"]
            and AD >= P["INDEL_LF_AD_MIN"]
            and HMER <= 4
            and LEN <= P["INS_MAX_LF"]
            and (sg in (None, "strand_unevaluable", "strand_fail_but_covered"))
        ):
            return TierResult(
                "INDEL-LF", "CANDIDATE", [] if sg in (None, "strand_unevaluable") else [sg]
            )
        ...
        if AF < P["AF_LOW"]:
            why.append("af_low")
        if DP < min(P["INDEL_LF_DP_MIN"], P["INDEL_MC_DP_MIN"]):
            why.append("dp_low")
        if AD < min(P["INDEL_LF_AD_MIN"], P["INDEL_MC_AD_MIN"]):
            why.append("ad_low")
        if HMER >= P["HMER_FAIL_LEN_INDEL"]:
            why.append("homopolymer")
        if LEN > P["INS_MAX_HC"]:
            why.append("len_too_long")
        if strand_status == "fail":
            why.append("strand_fail")
...
def mirror_indel_resolution(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (chrom, pos), g in sub.groupby(["chrom", "pos"]):
        ...
        for bi in set(ins_bases.values()):
            ...
            for j in secondary:
                ...
                if (
                    AF2 <= 0.03
                    or strand_status == "fail"
                    or float(df.loc[j, "ALT_QUAL"]) < P["ALT_QUAL_MIN"]
                ):
                    to_reject.add(j)
    ...
    if to_reject:
        ...
        df.loc[list(to_reject), "reason"] = (
            df.loc[list(to_reject), "reason"].astype(str) + ";mirror_indel_in_homopolymer"
        )
        ...
        while i < len(pos):
            ...
            if j - i >= P["CLUSTER_COUNT"]:
                ...
                m = (
                    df["chrom"].eq(chrom)
                    & df["pos"].between(s, e)
                    & df["var_type"].isin(["INS", "DEL"])
                    & (df["ALT_FREQ"] < P["AF_MID_LOW"])
                )
...
def apply_blacklist(df: pd.DataFrame, bl: set) -> pd.DataFrame:
    ...
    mask_mid = hit & (
        df["ALT_FREQ"].between(BLACKLIST_FORCE_AF, BLACKLIST_CAND_AF, inclusive="left")
    )
...
def main():
    ...
    # All mode
        ...
        if not args.yes:
            response = input(
                f"Process all {len(samples)} samples with profile '{args.profile}'? (y/n): "
            )
            ...
            bl_rows.append(
                {
                    ...
                    "ref": ref,
                    "alt": alt,
                    "n_samples_lowAF": len(sset),
                    "frac": round(len(sset) / n, 4),
                }
            )
    ...
    print(
        f"[INFO] Blacklist events: {len(bl_rows)} (AF≤{BLACKLIST_AF_MAX}, ≥{BLACKLIST_MIN_FRAC*100:.0f}% samples or ≥{BLACKLIST_MIN_COUNT_FLOOR} samples)"
    )
    ...
    pd.DataFrame(summary).to_csv(
        os.path.join(OUT_ROOT, "summary_counts.tsv"), sep="\t", index=False
    )
