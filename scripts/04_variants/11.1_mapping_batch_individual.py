...
FASTQ_DIR = os.environ.get("HBV_FASTQ_DIR", os.path.join(_PROJECT_DIR, "host_deconv_out_5"))
...
def run_mapping(fq_path, ref_fa, out_dir, sample_id, threads=2):
    ...
    cmd_minimap2 = ["minimap2", "-ax", "map-ont", "-t", str(threads), ref_fa, fq_path]
    ...
    cmd_sam2bam = ["samtools", "view", "-bS", sam_path]
...
def find_samples():
    ...
    for item in os.listdir(CONSENSUS_DIR):
        ...
        fastq_filename = (
            f"{sample_id}_subsampled.trimmed_filtered.porechop.viral_enriched.unmasked.fastq.gz"
        )
