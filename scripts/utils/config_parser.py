...
def main():
    ...
    parser.add_argument(
        "--get", metavar="KEY", help="Get single value using dot notation (e.g., project.work_dir)"
    )

    parser.add_argument(
        "--export", action="store_true", help="Export all config as shell variable assignments"
    )

    parser.add_argument(
        "--validate", action="store_true", help="Validate configuration and report errors"
    )

    parser.add_argument("--summary", action="store_true", help="Print configuration summary")

    parser.add_argument(
        "--json", action="store_true", help="Output as JSON (for --get with complex values)"
    )
