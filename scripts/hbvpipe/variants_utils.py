...
def assign_tier(
    ...
    if caller == "consensus" or af >= 0.95:
        return TierResult(
            tier=f"{var_type}-FLIPPED", decision="KEEP", reason="Consensus differs from reference"
        )

    # Check minimum depth
    if depth < min_depth:
        return TierResult(
            tier=f"{var_type}-LF", decision="CANDIDATE", reason=f"Low depth ({depth} < {min_depth})"
        )
    ...
    if af >= hc_min_af:
        ...
        if require_both_callers and caller != "both":
            return TierResult(
                tier=tier, decision="CANDIDATE", reason=f"Single caller only ({caller})"
            )

        return TierResult(tier=tier, decision="KEEP", reason=f"High confidence (AF={af:.2%})")

    elif af >= mc_min_af:
        return TierResult(
            tier=f"{var_type}-MC", decision="KEEP", reason=f"Medium confidence (AF={af:.2%})"
        )

    elif af >= lf_min_af:
        return TierResult(
            tier=f"{var_type}-LF", decision="CANDIDATE", reason=f"Low frequency (AF={af:.2%})"
        )
...
def merge_variant_calls(
    ivar_variants: List[Variant], clair3_variants: List[Variant], position_tolerance: int = 0
) -> List[Variant]:
    """
    ...
    for v in ivar_variants:
        key = (v.chrom, v.pos, v.ref, v.alt)
        merged[key] = Variant(
            chrom=v.chrom, pos=v.pos, ref=v.ref, alt=v.alt, af=v.af, depth=v.depth, caller="ivar"
        )
...
def filter_by_blacklist(
    variants: List[Variant], blacklist: Set[Tuple[str, int, str, str]], mark_only: bool = True
