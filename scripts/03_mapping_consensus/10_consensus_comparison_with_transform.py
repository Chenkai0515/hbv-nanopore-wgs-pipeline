...
CONSENSUS_DIR = os.environ.get("HBV_CONSENSUS_DIR", os.path.join(_PROJECT_DIR, "Medaka_consensus_8/r2"))
...
def process_single_sample(sample_id, ref_id, reference_seq, output_format='csv'):
    ...
    logger.info(f"Sample {sample_id}: ref={ref_id} (len={len(reference_seq)}), consensus len={len(consensus_seq)}")
    ...
    try:
        if output_format == 'csv':
            with open(summary_file, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(['sample_id', 'reference_id', 'ref_length', 'consensus_length', 'offset', 'differences', 'time'])
            ...
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write("sample_id\treference_id\tref_length\tconsensus_length\toffset\tdifferences\ttime\n")
                for result in results:
                    if result:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Consensus vs Reference Comparison with Coordinate Transform')
    ...
    parser.add_argument('--output-format', choices=['csv', 'tsv', 'json'], default='csv', help='Output format')
    parser.add_argument('--list', action='store_true', help='List all available samples')
    parser.add_argument('--offset', type=int, default=DEFAULT_OFFSET, help=f'Coordinate offset (default: {DEFAULT_OFFSET})')
    ...
    if args.test:
        ...
        if result:
            ...
            if result['differences']:
                ...
                for diff in result['differences'][:5]:
                    print(f"    pos {diff['original_position']} (rotated {diff['rotated_position']}): "
        ...
        with ProcessPoolExecutor(max_workers=args.jobs) as executor:
            ...
            for future in as_completed(future_to_sample):
                ...
                try:
                    ...
                    completed += 1
                    logger.info(f"Progress: {completed}/{len(samples)} ({100*completed/len(samples):.1f}%)")
                except Exception as e:
        ...
        print(f"  Average: {total_differences/len(successful_results):.1f}" if successful_results else "  Average: 0")
